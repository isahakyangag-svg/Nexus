...
import { useAdmin, ChatSession, ChatMessage, SupportChannel, FeedbackItem } from '../context/AdminState';
...
export default function AdminPanelOverlay() {
  ...
  // Categories: 'dashboard' | 'branding' | 'installer' | 'channels' | 'interceptor' | 'logger'
  const [activeCategory, setActiveCategory] = useState<'dashboard' | 'branding' | 'installer' | 'channels' | 'interceptor' | 'logger'>('dashboard');
  ...
  useEffect(() => {
    ...
    specCrc, specSha, specRuntime, downloadSteps, supportChannels
  ]);
    ...
    if (file) {
      ...
      reader.onload = () => {
        if (typeof reader.result === 'string') {
          setTempUploadedFileBase64(reader.result);
  ...
  const handleApplyCustomizations = () => {
    ...
    setUploadedFileBase64(tempUploadedFileBase64);
    ...
    const settingsPayload = {
      ...
      uploadedFileBase64: tempUploadedFileBase64,
      specCrc: tempSpecCrc,
      specSha: tempSpecSha,
    ...
    if (tempDownloadFileType === 'file' && tempUploadedFileBase64) {
      addLog(`[Синхронизация] Передаем файл "${tempUploadedFileName}" на сервер дистрибутивов...`);
      fetch('/api/upload-installer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          ...
          fileType: tempUploadedFileType,
          fileData: tempUploadedFileBase64
        })
      })
      .then(res => {
      ...
      .then(data => {
        addLog(`[УСПЕХ] Файл "${tempUploadedFileName}" весом ${(data.size / 1024).toFixed(1)} KB успешно сохранен на сервере.`);
        showToast('Файл успешно сохранен на сервере!', 'success');
      })
      .catch(err => {
        console.error('File synchronization failed:', err);
        addLog(`[ОШИБКА СИНХРОНИЗАЦИИ] Не удалось сохранить файл на сервере: ${err.message}`);
        showToast('Не удалось загрузить файл на сервер. Будет использоваться локальная копия браузера.', 'warning');
      });
    }
